...
select product_id, count(sale_id) as sales_per_product from sales
group by product_id;
...
SELECT c1.first_name AS customer1, c2.first_name AS customer2, c1.membership_status
FROM customers c1
...
select p.product_id, p.product_name, count(s.sale_id) as total_sales
from sales s
join products p on s.product_id = p.product_id
...
select c.first_name, c.last_name, s.sale_date, SUM(s.total_amount) as max_sale
from sales s
join customers c on s.customer_id = c.customer_id
where s.sale_date between '2023/01/01' and '2023/12/31'
...
select p.product_name, p.price, s.sale_date
from sales s
join products p on s.product_id = p.product_id
order by price desc 
limit 1;
...
select distinct count(customer_id) as num_customers
from sales 
...
select distinct count(s.customer_id) as num_customers
from sales s
join customers c on s.customer_id = c.customer_id
...
select count(sale_id) as dec_sales 
from sales
...
select p.product_name, s.product_id, i.stock_quantity
from products p
...
select c.first_name, c.last_name, c.registration_date,s.sale_date, p.product_name
from customers c
...
from products p
join sales s on s.product_id = p.product_id
group by p.product_name, sale_month, sale_year;
...
select c.first_name, c.last_name, SUM(s.total_amount) as total_purchase
from customers c
join sales s on s.customer_id = c.customer_id
group by c.first_name, c.last_name
...
select count(distinct s.product_id) as unique_product
from sales s
...
where not s.sale_date >= now() - INTERVAL '6 months';
